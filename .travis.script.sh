...
then
	...
	mvn -B -e -V -Dvaadin.testbench.developer.license=$TESTBENCH_LICENSE -Dvaadin.charts.developer.license=$CHARTS_LICENSE -Dsonar.verbose=true -Dsonar.analysis.mode=issues -Dsonar.github.repository=$TRAVIS_REPO_SLUG -Dsonar.host.url=$SONAR_HOST -Dsonar.github.oauth=$SONAR_GITHUB_OAUTH -Dsonar.login=$SONAR_LOGIN -Dsonar.github.pullRequest=$TRAVIS_PULL_REQUEST -Dsonar.exclusions=$SONAR_EXCLUSIONS org.jacoco:jacoco-maven-plugin:prepare-agent verify sonar:sonar
...
then
	...
	mvn -B -e -V -Dmaven.javadoc.skip=false -Dvaadin.testbench.developer.license=$TESTBENCH_LICENSE -Dvaadin.charts.developer.license=$CHARTS_LICENSE -Dsonar.verbose=true -Dsonar.analysis.mode=publish -Dsonar.host.url=$SONAR_HOST -Dsonar.login=$SONAR_LOGIN -Dsonar.exclusions=$SONAR_EXCLUSIONS verify sonar:sonar
fi
