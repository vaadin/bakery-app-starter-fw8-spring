package com.vaadin.starter.bakery.ui.dataprovider;
...
public class PageableDataProviderTest {
	...
	private void assertRangeToPageSizeAndNumber(int from, int to, int pageSize, int pageNumber) {
		int limit = to - from + 1;
		Pair<Integer, Integer> resultPageSizeAndNumber = PageableDataProvider.limitAndOffsetToPageSizeAndNumber(from,
				limit);
		Assert.assertEquals("Converting " + from + "-" + to, Pair.of(pageSize, pageNumber), resultPageSizeAndNumber);
