...
public class DashboardView extends DashboardViewDesign implements OrdersView {
	...
	private void initYearlySalesGraph() {
		...
		conf.addSeries(new MyListSeries(Integer.toString(now - 1), oneYearBack));
		conf.addSeries(new MyListSeries(Integer.toString(now - 2), twoYearsBack));
	...
	private void initProductSplitMonthlyGraph() {
		...
		LinkedHashMap<Product, Integer> deliveries = presenter.getDeliveriesPerProduct(today.getMonthValue(),
				today.getYear());

		final DataSeries series = new DataSeries();
		for (Entry<Product, Integer> entry : deliveries.entrySet()) {
			series.add(new DataSeriesItem(entry.getKey().getName(), entry.getValue()));
		}
	...
	private void initDeliveriesGraphs() {
		...
		yearConf.addSeries(new MyListSeries("deliveries", presenter.getDeliveriesPerMonth(today.getYear())));
		...
		monthConf.addSeries(
				new MyListSeries("deliveries", presenter.getDeliveriesPerDay(today.getMonthValue(), today.getYear())));
	...
	public void enter(ViewChangeEvent event) {
		...
		todayLabel.setContent(stats.getDeliveredToday() + "/" + stats.getDueToday());
		notAvailableLabel.setContent(Integer.toString(stats.getNotAvailableToday()));
		...
		notAvailableLabel.setStyleName("problem", stats.getNotAvailableToday() > 0);
	}
