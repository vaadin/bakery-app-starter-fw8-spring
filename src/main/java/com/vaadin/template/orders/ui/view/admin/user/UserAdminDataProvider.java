...
public class UserAdminDataProvider extends PageableDataProvider<User, Object> {
	...
	protected Page<User> fetchFromBackEnd(Query<User, Object> query, Pageable pageable) {
		if (filter == null) {
			...
			return repository.findByEmailLikeIgnoreCaseOrNameLikeIgnoreCaseOrderByEmail(filter, filter, pageable);
		}
	}
	...
	protected int sizeInBackEnd(Query<User, Object> query) {
		if (filter == null) {
			...
			return (int) repository.countByEmailLikeIgnoreCaseOrNameLikeIgnoreCase(filter, filter);
		}
	}
